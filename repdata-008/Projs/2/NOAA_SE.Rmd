...
We began by loading a range of required libraries.
...
We then downloaded the data file to a temporary directory, and finally, we loaded the data, ready for use.
...
Next, we carry out the collection by employing an R list as a dictionary-like data-structure, which has labels for events as **keys**, and the regular expressions for matching the levels of `EVTYPE` as **values**.
...
1. Remove levels **?**, **1** and **2** for `CROPDMGEXP` factor
2. Remove levels **?**, **-**, **+**, **1**, **2**, **3**, **4**, **5**, **6**, **7** and **8** for `PROPDMGEXP` factor as it is uncertain what information these contain, and they represent a very small amount of data.
